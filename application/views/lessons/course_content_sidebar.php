                    ...
                    <div class="tab-pane fade" id="liveclass" role="tabpanel" aria-labelledby="liveclass-tab" style="text-align: center;">
                        ...
                        if ($live_class->num_rows() > 0):
                            ...
                            <div style="padding: 30px 0px;">
                                ...
                                <a href="<?php echo $live_class['teams_attendee_url']; ?>" 
...
<script type="text/javascript">
    ...
    function handleCertificatePayment() {
        var certificate_price = "<?php echo $course_details['certificate_price']; ?>";

        var result = confirm("Do you want to pay N" + certificate_price + " for a Certificate?");

        if(result){
            <?php 
                $this->session->set_userdata('total_price_of_checking_out', $course_details['certificate_price']);
    ...
    function checkCertificateEligibility() {
        ...
        var is_free_certificate = "<?php echo $course_details['is_free_certificate']; ?>";

        if(if_live_class === 'true'){
            $.ajax({
                url: '<?php echo site_url('addons/certificate/check_live_class_certificate_eligibility/'.$course_id);?>',
                success: function(response)
                {
