...
class Liveclass extends CI_Controller{
    ...
    public function join($course_id = "") {
        ...
        $zoom_api_secret = get_settings('zoom_secret_key');
        $zoom_meeting_number = $this->liveclass_model->get_live_class_details('zoom_meeting_id');
        $zoom_role = 0;

        $page_data['logged_user_signature'] = $this->generate_signature($zoom_api_key, $zoom_api_secret, $zoom_meeting_number, $zoom_role);
